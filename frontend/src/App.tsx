...
import { CandleChart, type Candle, type MarketStructure } from './CandleChart';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const [nextAnalysis, rawCandles, nextDashboard, nextStructure, nextRanking, snapshot] = await Promise.all([
        ...
        setDashboard(nextDashboard); setStructure(nextStructure); setRanking(nextRanking); setLiveMarket(snapshot);
  ...
  const metrics = useMemo(() => [
    ...
    ['Active model', dashboard?.insights.activeModel?.version ?? 'No approved model'],
    ['Recent accuracy', percentage(summary?.accuracy)],
    ['Trade quality', quality ? `${Number(quality.tradeQualityScore).toFixed(1)} / 100` : '—'],
    ['Pending outcomes', String(summary?.pending ?? 0)],
  ], [analysis, dashboard, quality, summary]);
  ...
  return <main className="shell">
    ...
    <section className="hero-grid"><article className="panel chart-panel"><div className="panel-heading"><div><p className="eyebrow">{symbol} · {interval} · {structure?.structureState ?? 'BUILDING'}</p><h2>Market structure & validated trend lines</h2></div><div className="price-block"><strong>${money(liveMarket?.lastPrice ?? analysis?.indicators.price)}</strong><span>{structure?.regime ?? ranking?.broadMarketRegime ?? 'UNKNOWN'} · {candles.length} candles</span></div></div><CandleChart candles={candles} symbol={symbol} structure={structure} /><div className="chart-legend"><span className="ema20">EMA 20</span><span className="ema50">EMA 50</span><span className="vwap">VWAP</span><span>Support / resistance</span><span>ATR bands</span></div></article><aside className="panel intelligence-panel"><div className="panel-heading compact"><div><p className="eyebrow">Ensemble decision</p><h2>Rules + structure + ML</h2></div><span className={`grade grade-${(dashboard?.decision?.finalDirection ?? analysis?.decision ?? 'WAIT').toLowerCase()}`}>{dashboard?.decision?.finalDirection ?? analysis?.decision ?? 'WAIT'}</span></div><div className="metric-list">{metrics.map(([label, value]) => <div className="metric" key={label}><span>{label}</span><strong>{value}</strong></div>)}</div><p className="decision-reason">{dashboard?.decision?.reasons?.[0] ?? analysis?.reasons?.[0] ?? 'Waiting for validated data and an approved model.'}</p></aside></section>
